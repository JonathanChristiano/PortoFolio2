      ...
      {isAdmin ? (
        ...
        <div>
          ...
          <form onSubmit={isEditing ? handleUpdateProject : handleAddProject} className="space-y-4">
            <h1 className="text-2xl">Insert Your Projects</h1>
            ...
            {isEditing && (
              <button type="button" onClick={() => { setIsEditing(false); setNewProject({ name: '', description: '', link: '' }); }} className="px-4 py-2 bg-gray-500 text-white rounded">Cancel</button>
            )}
          </form>
            ...
            <ul>
              {projects.map((project) => (
                <li key={project.id} className="flex justify-between items-center border-b py-2">
                  ...
                  <div>
                    <button onClick={() => startEditingProject(project)} className="px-4 py-1 bg-yellow-500 text-white rounded mr-2">Edit</button>
